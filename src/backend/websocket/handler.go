...
func HandleTreeWebSocket(controller *elementsController.ElementController) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			defer wg.Done()
			if req.UseBFS {
				tree, nodesVisited, duration = elementsController.StartBFS(req.Target, req.Count, treeChan)
			} else {
				tree, nodesVisited, duration = elementsController.StartDFS(req.Target, req.Count, treeChan)
			}
			close(treeChan)
		}()
